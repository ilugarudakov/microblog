...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Изменения сохранены успешно!')
...
def follow(username):
    ...
    if user == current_user:
        flash('Подписка на себя невозможна')
        return redirect(url_for('user', username=username))
    current_user.follow(user)
    db.session.commit()
    flash('Вы успешно подписались на пользователя {}!.'.format(username))
    return redirect(url_for('user', username=username))
...
def unfollow(username):
    ...
    if user is None:
        ...
        return redirect(url_for('index'))
    if user == current_user:
        flash('Вы не можете отписываться от себя')
        return redirect(url_for('user', username=username))
    current_user.unfollow(user)
    db.session.commit()
    flash('Вы успешно отписались от пользователя {}!'.format(username))
    return redirect(url_for('user', username=username))
